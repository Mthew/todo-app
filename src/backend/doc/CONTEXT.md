...
- `GetTasksByUserUseCase` - Retrieve all user tasks with relations
...
## 8. Data Flow Example: Creating a New Task

1. **API Layer:** `POST /api/tasks` → `TaskController.create()`
2. **Validation:** Zod validates request body against `CreateTaskSchema`
3. **Authentication:** JWT middleware extracts user from token
4. **Application Layer:** `CreateTaskUseCase.execute()` with validated DTO
5. **Domain Layer:** Creates `Task` entity with business rules
6. **Infrastructure Layer:** `PrismaTaskRepository` saves to database
7. **Response:** Returns created task data directly (no wrapper)
...
- **Database Indexing**: Add indexes for userId foreign keys
...
- **Task Filtering**: Advanced filtering by priority, due date, category
- **Pagination**: Implement cursor-based pagination for large datasets
- **Task Attachments**: File upload capability for tasks
- **Team Collaboration**: Multi-user task sharing and permissions

This architecture ensures maintainability, testability, and scalability while providing a clear separation of concerns that makes the codebase easy to understand and extend.
